...
hang() {
    ...
    if [[ ${#CMD[@]} -ge 2 && "${CMD[$MAIN]}" == "sudo" && "${CMD[$SPAR]}" == "-i" ]]; then
        if [[ "$SUDOWARN" == "True" ]]; then
            cat << EOF
For security purposes, sudo -i will not run.
To run commands with root privileges, run NNOS Shell as root.
To disable this message, set SUDOWARN to False in the config file.
EOF
            else
                echo
            fi
    ...
    elif [[ "${CMD[@]}" == "ns show startpwd" ]]; then
        echo "$STARTPWD"
    elif [[ "${CMD[@]}" == "ns" ]]; then
        echo
